...
function ComplaintsPage() {
  ...
  return <main className="page-wrap">
    ...
    <AnimatePresence>{notice && <motion.div className="notice" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} onAnimationComplete={() => window.setTimeout(() => setNotice((current) => current === notice ? null : current), 3600)} data-testid="status-notice">{notice.text}</motion.div>}</AnimatePresence>
  </main>;
}
